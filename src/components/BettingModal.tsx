...
const weatherOptions: { type: WeatherType; icon: React.ReactNode; label: string }[] = [
  { type: "sunny", icon: <Sun className="w-5 h-5" />, label: "晴天" },
  { type: "rain", icon: <CloudRain className="w-5 h-5" />, label: "小雨" },
  { type: "drought", icon: <Flame className="w-5 h-5" />, label: "干旱" },
  { type: "flood", icon: <Waves className="w-5 h-5" />, label: "洪涝" },
  { type: "typhoon", icon: <Wind className="w-5 h-5" />, label: "台风" },
];

const quickAmounts = [10, 50, 100, 500];

const BettingModal = ({ province, open, onOpenChange }: BettingModalProps) => {
                  ...
                  <div
                    ...
                    ${
                      option.type === "sunny"
                        ...
                        ? "bg-weather-rain/30"
                        : option.type === "drought"
                        ? "bg-weather-drought/30"
                        : option.type === "flood"
                        ? "bg-weather-flood/30"
                        : "bg-weather-typhoon/30"
                    }
                  `}
                  >
                    {option.icon}
